...
function parseMechanics(html,item){
  ...
  const condition=saveCondition(body),repeatSave=repeatSaveFact(body);let saveFailure="",saveSuccess="";
  ...
  return {area,damage:healing?"":damage,healing,damageType,save:saveName?SAVE_NAMES[saveName]:"",saveFailure,saveSuccess,condition,repeatSave,attack,mechanics:result.length?`${result.join(" ")}`:"Aplique o efeito aos alvos indicados.",higherLevels};
}
async function loadMechanics(spells){
  const cacheUrl=new URL("spell-mechanics-cache.json",import.meta.url),version=8;let cache={};try{cache=JSON.parse(await readFile(cacheUrl,"utf8"));if(cache.__version!==version)cache={__version:version}}catch{cache={__version:version}}
...
const spells=raw.map(item=>{
  ...
  const description=effectFor(spell);return {key:item.key,title:`${ICON[item.school]||"✨"} ${displayName}`,reference:item.reference,level:item.level,kind:"completa",status:"teste",classes:[...(classes.get(slug(item.reference))||[])],school:schoolData[0],source:schoolData[0],manifestation:schoolData[1],cast:translateTime(item.castRaw),range,duration:translateDuration(item.durationRaw),components:item.components,area:rule.area||areaFrom(range),materialIds,conjuration:conjurationText(item,materialIds),description,effect:`${description} ${rule.mechanics||""}`.trim(),limitation:"A magia respeita cobertura, Concentração, imunidades e requisitos de alvo indicados.",damage:rule.damage||"",healing:rule.healing||"",damageType:rule.damageType||"",attack:!!rule.attack,save:rule.save||"",saveFailure:rule.saveFailure||"",saveSuccess:rule.saveSuccess||"",condition:rule.condition||"",repeatSave:rule.repeatSave||"",higherLevels:rule.higherLevels||"",flags:item.flags};
});
