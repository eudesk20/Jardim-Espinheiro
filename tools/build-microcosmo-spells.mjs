...
function parseMechanics(html,item){
  ...
  else if(saveName){result.push(`O alvo faz uma Salvaguarda de ${SAVE_NAMES[saveName]}`);if(damage){result.push(`Se falhar, sofre ${damage} de dano`);result.push(/half as much damage|half damage/i.test(body)?"Se passar, sofre metade do dano":"Se passar, não sofre o dano")}else{result.push("Se falhar, sofre o efeito descrito");result.push("Se passar, evita o efeito")}}
  else if(damage)result.push(`O alvo sofre ${damage} de dano`);
  if(healing)result.push(`A criatura recupera ${healing} pontos de vida`);
  return {area,damage:healing?"":damage,healing,damageType,save:saveName?SAVE_NAMES[saveName]:"",attack,mechanics:result.length?`${result.join(". ")}.`:"Aplique o efeito aos alvos indicados.",higherLevels};
}
async function loadMechanics(spells){
  const cacheUrl=new URL("spell-mechanics-cache.json",import.meta.url),version=6;let cache={};try{cache=JSON.parse(await readFile(cacheUrl,"utf8"));if(cache.__version!==version)cache={__version:version}}catch{cache={__version:version}}
