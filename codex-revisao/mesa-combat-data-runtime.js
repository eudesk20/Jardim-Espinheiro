...
(async function(){
  ...
  async function connect(){try{supabase=await globalThis.MICROCOSMOS_GET_SUPABASE();const {data:{session}}=await supabase.auth.getSession();currentUserId=session?.user?.id||""}catch(e){console.warn("MICROCOSMOS Mesa: dados completos online indisponíveis",e)}}
