/* MICROCOSMOS — Dados completos de combate para a Mesa.
   ...
   ataque mágico e metadados completos de armas/magias para o executor. */
(async function(){
  ...
  function buildSpells(data){const caster=classCaster(data),spellAttack=mod(data.stats?.[caster])+prof(data.level),spellDc=8+spellAttack;return(data.magic?.known||[]).map(s=>({name:s.name||"Magia",codexKey:s.codexKey||"",reference:s.reference||"",lvl:spellLevel(s),bonus:s.attack?spellAttack:0,spellDc,damage:s.damage||"",healing:s.healing||"",kind:s.healing?"cura":s.attack?"ataque":s.save?"salvaguarda":"efeito",attack:!!s.attack,save:s.save||"",range:s.range||"",area:s.area||"",shape:s.shape||"",damageType:s.damageType||damageType(`${s.damage||""} ${s.text||""} ${s.effect||""}`),text:s.text||s.description||"",effect:s.effect||"",limitation:s.limitation||"",higherLevels:s.higherLevels||"",components:s.comp||s.components||[],consumed:!!s.consumed,requiredMaterialCost:+(s.requiredMaterialCost||s.componentCost||0)||0}))}
  function applyData(token,data){if(!token||!data)return;const caster=classCaster(data),spellAttack=mod(data.stats?.[caster])+prof(data.level);token.stats={FOR:+data.stats?.FOR||10,DES:+data.stats?.DES||10,CON:+data.stats?.CON||10,INT:+data.stats?.INT||10,SAB:+data.stats?.SAB||10,CAR:+data.stats?.CAR||10};token.saveProficiencies=[...(data.saves||[])];token.saveBonuses=Object.fromEntries(["FOR","DES","CON","INT","SAB","CAR"].map(a=>[a,saveBonus(data,a)]));token.spellAbility=caster;token.spellAttack=spellAttack;token.spellDc=8+spellAttack;token.size=data.size||data.creatureSize||"Pequeno";token.resistances=[...(data.resistances||[])];token.vulnerabilities=[...(data.vulnerabilities||[])];token.immunities=[...(data.immunities||[])];token.conditionImmunities=[...(data.conditionImmunities||[])];token.conditions=[...(data.conditions||[])];token.attacks=buildAttacks(data);token.spells=buildSpells(data);token.combatDataReady=true}
  ...
  async function refresh(){
    const rows=[];if(supabase&&currentUserId){const {data,error}=await supabase.from("characters").select("id,user_id,data");if(!error)rows.push(...(data||[]))}
    const local=readLocal();if(Object.keys(local).length){const own=players.find(p=>p.userId===currentUserId&&p.linked)||players.find(p=>p.linked&&String(p.characterId||"").startsWith("local-"));if(own)applyData(own,local)}
    for(const p of players.filter(x=>x.linked)){const row=rows.find(r=>r.id===p.characterId)||rows.find(r=>r.user_id===p.userId);if(row?.data)applyData(p,row.data)}
    try{globalThis.MICROCOSMOS_TABLE_API?.renderPlayers?.();globalThis.MICROCOSMOS_TABLE_API?.renderTokens?.();if(document.querySelector("#tokenLayer .token.selected")){const id=document.querySelector("#tokenLayer .token.selected")?.dataset.token;globalThis.MICROCOSMOS_TABLE_API?.selectToken?.(id)}}catch(e){}
    return players
  }
  globalThis.MICROCOSMOS_TABLE_COMBAT_DATA={refresh,applyData,buildAttacks,buildSpells};
  await connect();await refresh();window.addEventListener("storage",e=>{if(e.key===SHEET_KEY)refresh()});document.addEventListener("visibilitychange",()=>{if(!document.hidden)refresh()});
})();
