...
(async function(){
  ...
  async function reviewApproval(id,approve){const {error}=await sb.rpc("review_interaction",{interaction_id:id,approve,note:null});if(error){alert("Não foi possível revisar: "+error.message);return}try{await globalThis.MICROCOSMOS_TABLE_COMBAT_DATA?.refresh?.();await globalThis.MICROCOSMOS_MESA_SHARED?.reloadTokens?.()}catch(_e){}await renderApprovals()}
