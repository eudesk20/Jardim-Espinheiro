/* MICROCOSMOS — Atlas da Campanha v1: Pastas + Cenas independentes. */
(async function(){
  ...
  function load(){
    ...
    const folderId=uid("folder"),sceneId=uid("scene");return{version:1,activeSceneId:sceneId,folders:[{id:folderId,name:"Campanha",open:true}],scenes:[{id:sceneId,folderId,name:"Cena Atual",geometry:readGeometry(),gridType:gridType?.value||"square",gridSize:+gridSize?.value||70,mapData:"",updatedAt:Date.now()}]}
  }
  let atlas=load();
  function save(){localStorage.setItem(STORE,JSON.stringify(atlas))}
  function active(){return atlas.scenes.find(s=>s.id===atlas.activeSceneId)||atlas.scenes[0]}
  function captureCurrent(){const s=active();if(!s)return;s.geometry=readGeometry();s.gridType=gridType?.value||s.gridType||"square";s.gridSize=+gridSize?.value||s.gridSize||70;s.updatedAt=Date.now();save()}
  ...
  function applySceneVisuals(){
    ...
    if(mapImage){if(s.mapData){mapImage.src=s.mapData;mapImage.hidden=false;if(mapStatus)mapStatus.textContent=`Cena: ${s.name}`}else{mapImage.hidden=true;mapImage.removeAttribute("src");if(mapStatus)mapStatus.textContent=`Cena: ${s.name} • sem mapa`}}
  }
  function switchScene(id){
    if(id===atlas.activeSceneId)return;captureCurrent();const next=atlas.scenes.find(s=>s.id===id);if(!next)return;atlas.activeSceneId=id;save();localStorage.setItem(SCENE_STORE,JSON.stringify(next.geometry||emptyGeometry()));location.reload()
  }
  function addFolder(){const name=prompt("Nome da Pasta / Conjunto:","Novo Conjunto");if(!name?.trim())return;atlas.folders.push({id:uid("folder"),name:name.trim(),open:true});save();render()}
  function addScene(folderId){const name=prompt("Nome da nova Cena:","Nova Cena");if(!name?.trim())return;captureCurrent();const id=uid("scene");atlas.scenes.push({id,folderId,name:name.trim(),geometry:emptyGeometry(),gridType:gridType?.value||"square",gridSize:+gridSize?.value||70,mapData:"",updatedAt:Date.now()});atlas.activeSceneId=id;save();localStorage.setItem(SCENE_STORE,JSON.stringify(emptyGeometry()));location.reload()}
  function renameFolder(id){const f=atlas.folders.find(x=>x.id===id);if(!f)return;const name=prompt("Renomear Pasta:",f.name);if(!name?.trim())return;f.name=name.trim();save();render()}
  function renameScene(id){const s=atlas.scenes.find(x=>x.id===id);if(!s)return;const name=prompt("Renomear Cena:",s.name);if(!name?.trim())return;s.name=name.trim();save();render();applySceneVisuals()}
  function deleteScene(id){const s=atlas.scenes.find(x=>x.id===id);if(!s||atlas.scenes.length<=1){alert("O Atlas precisa manter pelo menos uma Cena.");return}if(!confirm(`Excluir a Cena “${s.name}”?`))return;atlas.scenes=atlas.scenes.filter(x=>x.id!==id);if(atlas.activeSceneId===id){const next=atlas.scenes[0];atlas.activeSceneId=next.id;localStorage.setItem(SCENE_STORE,JSON.stringify(next.geometry||emptyGeometry()));save();location.reload();return}save();render()}
  function deleteFolder(id){const f=atlas.folders.find(x=>x.id===id);if(!f)return;const scenes=atlas.scenes.filter(s=>s.folderId===id);if(scenes.length){alert("Mova ou exclua as Cenas desta Pasta antes de apagá-la.");return}if(!confirm(`Excluir a Pasta “${f.name}”?`))return;atlas.folders=atlas.folders.filter(x=>x.id!==id);save();render()}
  function moveScene(id){const s=atlas.scenes.find(x=>x.id===id);if(!s)return;const options=atlas.folders.map((f,i)=>`${i+1}. ${f.name}`).join("\n"),answer=prompt(`Mover “${s.name}” para qual Pasta?\n${options}`);const n=+answer;if(!Number.isInteger(n)||n<1||n>atlas.folders.length)return;s.folderId=atlas.folders[n-1].id;save();render()}
  ...
  function render(){
    ...
    for(const f of atlas.folders){const scenes=atlas.scenes.filter(s=>s.folderId===f.id),d=document.createElement("details");d.className="micro-atlas-folder";d.open=f.open!==false;d.innerHTML=`<summary><span>${d.open?"▼":"▶"}</span><span>📁 ${esc(f.name)}</span><span class="grow"></span><span class="micro-atlas-badge">${scenes.length}</span><button class="micro-atlas-mini" data-add-scene title="Nova Cena">＋</button><button class="micro-atlas-mini" data-folder-edit title="Renomear">✎</button><button class="micro-atlas-mini" data-folder-delete title="Excluir">🗑</button></summary><div class="micro-atlas-folder-body"></div>`;const body=d.querySelector(".micro-atlas-folder-body");
      ...
      d.addEventListener("toggle",()=>{f.open=d.open;save();const icon=d.querySelector("summary>span");if(icon)icon.textContent=d.open?"▼":"▶"});d.querySelector("[data-add-scene]").onclick=e=>{e.preventDefault();e.stopPropagation();addScene(f.id)};d.querySelector("[data-folder-edit]").onclick=e=>{e.preventDefault();e.stopPropagation();renameFolder(f.id)};d.querySelector("[data-folder-delete]").onclick=e=>{e.preventDefault();e.stopPropagation();deleteFolder(f.id)};host.appendChild(d)
    }
    panel.querySelector("[data-atlas-folder]").onclick=addFolder
  }

  // Migração: a primeira Cena preserva o cenário que já existia antes do Atlas.
  const current=active();if(current&&!current.geometry)current.geometry=readGeometry();save();
  applySceneVisuals();render();
  gridType?.addEventListener("change",()=>{const s=active();if(s){s.gridType=gridType.value;save()}});
  gridSize?.addEventListener("input",()=>{const s=active();if(s){s.gridSize=+gridSize.value||70;save()}});
  mapFile?.addEventListener("change",async e=>{const file=e.target.files?.[0];if(!file)return;try{const data=await compressMap(file),s=active();if(s){s.mapData=data;s.updatedAt=Date.now();save();setTimeout(applySceneVisuals,0)}}catch(err){console.warn("MICROCOSMOS Atlas: mapa não pôde ser persistido",err)}},true);
  $("clearMap")?.addEventListener("click",()=>{const s=active();if(s){s.mapData="";save()}},true);
  document.addEventListener("microcosmos:scene-changed",()=>{clearTimeout(globalThis.__microAtlasSaveTimer);globalThis.__microAtlasSaveTimer=setTimeout(captureCurrent,180)});
  window.addEventListener("beforeunload",captureCurrent);
  globalThis.MICROCOSMOS_ATLAS={get data(){return atlas},active,save:captureCurrent,switchScene,addFolder,addScene,render};
})();
