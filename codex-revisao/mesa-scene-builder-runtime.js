...
(async function(){
  ...
  function elementSvg(el){
    ...
    if(el.shape==="polyline"&&Array.isArray(el.points)){
      const d=pathD(el.points,el.cornerStyle==="round",el.cornerControls),handles=editingHandles?el.points.map((p,i)=>{let extra="";if(i>0&&i<el.points.length-1){const g=cornerGeom(el.points,i,el.cornerControls,el.cornerStyle==="round"),both={x:(g.before.x+g.after.x)/2,y:(g.before.y+g.after.y)/2};extra=`<path class="micro-scene-guide" d="M ${g.before.x} ${g.before.y} L ${p.x} ${p.y} L ${g.after.x} ${g.after.y}"></path><circle class="micro-scene-handle" data-scene-handle="corner-side" data-corner-side="in" data-point-index="${i}" cx="${g.before.x}" cy="${g.before.y}" r="7"></circle><circle class="micro-scene-handle" data-scene-handle="corner-side" data-corner-side="out" data-point-index="${i}" cx="${g.after.x}" cy="${g.after.y}" r="7"></circle><circle class="micro-scene-handle corner-both" data-scene-handle="corner-both" data-point-index="${i}" cx="${both.x}" cy="${both.y}" r="7"></circle>`}return`${extra}<circle class="micro-scene-handle endpoint" data-scene-handle="point" data-point-index="${i}" cx="${p.x}" cy="${p.y}" r="9"></circle>`}).join(""):"";
  ...
  function finishPen(){
    ...
    const el=normalize({id:uid(),type:"wall",shape:"polyline",layer:d.layer,state:"solid",cornerStyle:d.cornerStyle||"straight",cornerControls:{},points:points.map(p=>({x:+p.x.toFixed(1),y:+p.y.toFixed(1)}))});scene.elements.push(el);selectedId=el.id;save();setTool("select");render()
  }
  function eraseAt(e){
