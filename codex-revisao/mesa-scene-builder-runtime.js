...
(async function(){
  ...
  function elementSvg(el){
    ...
    if(el.type==="light"){
      const angle=Number.isFinite(+el.direction)?+el.direction:0,arc=el.mode==="directional"?Math.max(15,+el.arc||70):360,r=Math.max(15,+el.r||70),a1=(angle-arc/2)*Math.PI/180,a2=(angle+arc/2)*Math.PI/180,path=arc>=359?"":`M ${el.cx} ${el.cy} L ${el.cx+Math.cos(a1)*r} ${el.cy+Math.sin(a1)*r} A ${r} ${r} 0 ${arc>180?1:0} 1 ${el.cx+Math.cos(a2)*r} ${el.cy+Math.sin(a2)*r} Z`,gradient=`microLightGlow-${String(el.id).replace(/[^a-z0-9_-]/gi,"")}`,shape=arc>=359?`<circle class="micro-light-area${sel}" cx="${el.cx}" cy="${el.cy}" r="${r}" style="fill:url(#${gradient})"></circle>`:`<path class="micro-light-area${sel}" d="${path}" style="fill:url(#${gradient})"></path>`,handles=editingHandles?`<circle class="micro-scene-handle" data-scene-handle="light-move" cx="${el.cx}" cy="${el.cy}" r="10"></circle><circle class="micro-scene-handle endpoint" data-scene-handle="light-range" cx="${el.cx+Math.cos(angle*Math.PI/180)*r}" cy="${el.cy+Math.sin(angle*Math.PI/180)*r}" r="9"></circle>`:"";
      return `<g data-scene-id="${esc(el.id)}"><defs><radialGradient id="${gradient}" gradientUnits="userSpaceOnUse" cx="${el.cx}" cy="${el.cy}" r="${r}"><stop offset="0" stop-color="#fff8c9" stop-opacity=".78"/><stop offset=".22" stop-color="#ffd45d" stop-opacity=".48"/><stop offset=".58" stop-color="#ffc947" stop-opacity=".2"/><stop offset="1" stop-color="#ffbd36" stop-opacity="0"/></radialGradient></defs><circle class="micro-scene-hit" cx="${el.cx}" cy="${el.cy}" r="18" fill="none"></circle>${shape}<circle class="micro-light-source" cx="${el.cx}" cy="${el.cy}" r="9"></circle>${handles}${dynamicControlSvg(el)}</g>`
    }
    if(el.type==="circle"){
