...
(async function(){
  ...
  function elementSvg(el){
    ...
    if(el.type==="light"){
      ...
      return `<g data-scene-id="${esc(el.id)}"><defs><radialGradient id="${gradient}" gradientUnits="userSpaceOnUse" cx="${el.cx}" cy="${el.cy}" r="${r}"><stop offset="0" stop-color="#fff8c9" stop-opacity=".78"/><stop offset=".22" stop-color="#ffd45d" stop-opacity=".48"/><stop offset=".58" stop-color="#ffc947" stop-opacity=".2"/><stop offset="1" stop-color="#ffbd36" stop-opacity="0"/></radialGradient></defs><circle class="micro-scene-hit" cx="${el.cx}" cy="${el.cy}" r="${Math.max(16,r)}" fill="none"></circle>${shape}<circle class="micro-light-source" cx="${el.cx}" cy="${el.cy}" r="9"></circle>${handles}</g>`
    }
    if(el.type==="circle"){
