...
(async function(){
  ...
  function partialEraseAt(e){if(!isMaster||tool!=="partialErase")return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();if(e.type==="pointerdown"){partialErasing=e.pointerId;try{stage.setPointerCapture?.(e.pointerId)}catch(_e){}}else if(partialErasing!==e.pointerId)return;const p=stagePoint(e),size=+$("microEraserSize")?.value||70,shape=$("microEraserShape")?.value||"circle";showEraserPreview(p,size,shape);let changed=false,next=[];for(const el of scene.elements){if(!(el.type==="circle"||el.type==="wall")){next.push(el);continue}const pts=sampledPoints(el);if(!pts.some(q=>brushContains(q,p,size,shape))){next.push(el);continue}changed=true;let run=[];const flush=()=>{if(run.length>1){const clean=simplifyPoints(run,3);if(clean.length>1)next.push(normalize({id:uid(),groupId:el.groupId,type:"wall",shape:"polyline",layer:el.layer,state:"solid",cornerStyle:"custom",cornerControls:{},generatedFromErase:true,points:clean}))}run=[]};for(const q of pts){if(brushContains(q,p,size,shape))flush();else run.push({x:+q.x.toFixed(1),y:+q.y.toFixed(1)})}flush()}if(changed){scene.elements=next;autoFuseGroups();selectedId="";save();render()}}
  ...
  if(isMaster){
    ...
    stage.addEventListener("pointerdown",partialEraseAt,true);stage.addEventListener("pointermove",partialEraseAt,true);stage.addEventListener("pointerup",e=>{if(partialErasing===e.pointerId){partialErasing=null;try{stage.releasePointerCapture?.(e.pointerId)}catch(_e){}}},true);stage.addEventListener("pointercancel",e=>{if(partialErasing===e.pointerId)partialErasing=null},true);stage.addEventListener("pointerleave",()=>{if(partialErasing===null)$("microPartialEraserPreview")?.remove()},true);
