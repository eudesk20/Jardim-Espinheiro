...
(async function(){
  ...
  function save(record=true){try{const next=JSON.stringify(scene);if(record&&next!==lastSnapshot){undoStack.push(lastSnapshot);if(undoStack.length>60)undoStack.shift()}lastSnapshot=next;localStorage.setItem(STORE_KEY,next);updateUndoButton();globalThis.MICROCOSMOS_VISION?.refresh?.();document.dispatchEvent(new CustomEvent("microcosmos:scene-changed"));setTimeout(()=>globalThis.MICROCOSMOS_MESA_SHARED?.flushScene?.(),0)}catch(e){console.warn("MICROCOSMOS: não foi possível salvar cenário",e)}}
  function updateUndoButton(){const b=$("microSceneUndo");if(b){b.disabled=!undoStack.length;b.title=undoStack.length?`Desfazer última alteração (${undoStack.length} disponível${undoStack.length===1?"":"is"})`:"Nenhuma alteração para desfazer"}const mobile=$("microSceneMobileUndo");if(mobile)mobile.disabled=!undoStack.length}
  ...
  function buildPanel(){
    ...
    left.appendChild(panel);const lightButton=document.createElement("button");lightButton.type="button";lightButton.className="btn";lightButton.dataset.sceneTool="light";lightButton.textContent="💡 Fonte de Luz";panel.querySelector(".micro-builder-tools")?.appendChild(lightButton);$("microBuilderStatus")?.insertAdjacentHTML("beforebegin",`<div class="micro-builder-row"><label>Formato da Luz<select id="microLightMode"><option value="radial">Todas as direções</option><option value="directional">Direcional</option></select></label><label>Cor da nova luz<input id="microLightColor" type="color" value="#ffd86b"></label></div><div class="micro-builder-row"><label>Ângulo direcional<input id="microLightArc" type="number" min="15" max="180" step="5" value="70"></label></div><label style="display:block;margin-top:7px">Nível de edição<select id="microSelectLevel"><option value="all">Todos os elementos</option><option value="structure">Paredes e círculos</option><option value="openings">Portas e janelas</option><option value="lights">Somente luzes</option></select></label><small>Setas movem a seleção; Shift + seta usa meio quadrado.</small>`);
    const rows=[...panel.querySelectorAll(":scope > .micro-builder-row")],advanced=document.createElement("details");advanced.id="microSceneAdvanced";advanced.className="micro-builder-advanced";advanced.innerHTML='<summary>⚙️ Configurações avançadas</summary><div class="micro-builder-advanced-body"></div>';const advancedBody=advanced.querySelector("div"),statusBox=$("microBuilderStatus");for(const row of rows.slice(1))advancedBody.appendChild(row);const level=$("microSelectLevel")?.closest("label"),hint=level?.nextElementSibling;if(level)advancedBody.appendChild(level);if(hint?.tagName==="SMALL")advancedBody.appendChild(hint);statusBox?.insertAdjacentElement("beforebegin",advanced);
    ...
    });$("microSceneUndo").addEventListener("click",undoScene);$("microBuilderLayer").addEventListener("change",e=>targetLayer=e.target.value);$("microSelectLevel").addEventListener("change",e=>{selectLevel=e.target.value;selectedId="";render()});setTool("select");updateUndoButton()
  }
