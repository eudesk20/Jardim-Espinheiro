...
(async function(){
  ...
  function elementSvg(el){
    ...
    if(el.type==="light"){
      const angle=Number.isFinite(+el.direction)?+el.direction:0,arc=el.mode==="directional"?Math.max(15,+el.arc||70):360,r=Math.max(15,+el.r||70),color=/^#[0-9a-f]{6}$/i.test(el.color||"")?el.color:"#ffd86b",a1=(angle-arc/2)*Math.PI/180,a2=(angle+arc/2)*Math.PI/180,path=arc>=359?"":`M ${el.cx} ${el.cy} L ${el.cx+Math.cos(a1)*r} ${el.cy+Math.sin(a1)*r} A ${r} ${r} 0 ${arc>180?1:0} 1 ${el.cx+Math.cos(a2)*r} ${el.cy+Math.sin(a2)*r} Z`,gradient=`microLightGlow-${String(el.id).replace(/[^a-z0-9_-]/gi,"")}`,shape=arc>=359?`<circle class="micro-light-area${sel}" cx="${el.cx}" cy="${el.cy}" r="${r}" style="fill:url(#${gradient});stroke:${color}"></circle>`:`<path class="micro-light-area${sel}" d="${path}" style="fill:url(#${gradient});stroke:${color}"></path>`,handles=editingHandles?`<circle class="micro-scene-handle" data-scene-handle="light-move" cx="${el.cx}" cy="${el.cy}" r="10"></circle><circle class="micro-scene-handle endpoint" data-scene-handle="light-range" cx="${el.cx+Math.cos(angle*Math.PI/180)*r}" cy="${el.cy+Math.sin(angle*Math.PI/180)*r}" r="9"></circle>`:"";
      return `<g data-scene-id="${esc(el.id)}"><defs><radialGradient id="${gradient}" gradientUnits="userSpaceOnUse" cx="${el.cx}" cy="${el.cy}" r="${r}"><stop offset="0" stop-color="${color}" stop-opacity=".82"/><stop offset=".22" stop-color="${color}" stop-opacity=".5"/><stop offset=".58" stop-color="${color}" stop-opacity=".2"/><stop offset="1" stop-color="${color}" stop-opacity="0"/></radialGradient></defs><circle class="micro-scene-hit" cx="${el.cx}" cy="${el.cy}" r="18" fill="none"></circle>${shape}<circle class="micro-light-source" cx="${el.cx}" cy="${el.cy}" r="9" style="fill:${color}"></circle>${handles}${dynamicControlSvg(el)}</g>`
    }
    if(el.type==="circle"){
  ...
  function renderSelected(){
    ...
    if(el.type==="light"){
      ...
      box.innerHTML=`<b>💡 Fonte de luz</b><span class="micro-layer-chip">VISÍVEL</span><div class="micro-builder-row"><label>Formato<select id="microElementLightMode"><option value="radial" ${el.mode!=="directional"?"selected":""}>Todas as direções</option><option value="directional" ${el.mode==="directional"?"selected":""}>Direcional</option></select></label><label>Cor<input id="microElementLightColor" type="color" value="${color}"></label></div><div class="micro-builder-row"><label>Distância<input id="microElementLightRange" type="number" min="1.5" max="90" step="1.5" value="${meters}"></label><label>Ângulo<input id="microElementLightArc" type="number" min="15" max="180" step="5" value="${+el.arc||70}"></label></div><div class="micro-builder-row"><label>Estado<select id="microElementLightState"><option value="on" ${el.state!=="off"?"selected":""}>Acesa</option><option value="off" ${el.state==="off"?"selected":""}>Apagada</option></select></label></div><small>Arraste o centro para mover e o ponto amarelo para mudar direção e distância.</small><button type="button" class="btn danger" id="microDeleteElement" style="width:100%;margin-top:6px">🗑️ Apagar fonte</button>`;
  ...
  function buildPanel(){
    ...
    const panel=document.createElement("section");panel.className="panel micro-builder-panel";panel.id="microSceneBuilderPanel";panel.innerHTML=`<h3>🛠️ Construir Cenário</h3>
      ...
      <div class="micro-builder-groups">
        ...
        <details class="micro-builder-group" id="microSceneLightGroup"><summary>💡 Fonte de Luz e visão</summary><div class="micro-builder-group-body" id="microSceneLightConfig"><button type="button" class="btn" data-scene-tool="light" style="width:100%">💡 Criar Fonte de Luz</button><div class="micro-builder-row"><label>Formato<select id="microLightMode"><option value="radial">Todas as direções</option><option value="directional">Direcional</option></select></label><label>Cor<input id="microLightColor" type="color" value="#ffd86b"></label></div><div class="micro-builder-row"><label>Ângulo<input id="microLightArc" type="number" min="15" max="180" step="5" value="70"></label></div></div></details>
    ...
    });$("microSceneUndo").addEventListener("click",undoScene);$("microSceneRedo").addEventListener("click",redoScene);$("microBuilderLayer").addEventListener("change",e=>targetLayer=e.target.value);$("microSelectLevel").addEventListener("change",e=>{selectLevel=e.target.value;selectedId="";render()});setTool("select");updateUndoButton()
  }
