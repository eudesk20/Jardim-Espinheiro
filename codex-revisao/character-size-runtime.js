...
(function(){
  ...
  function raceSize(){
    try{return norm(globalThis.CODEX_RACE_DATA?.[state.race]?.size||globalThis.MICROCOSMO_DATA?.races?.[state.race]?.size||"Médio")}catch{return"Médio"}
  }
  function resolveBase(){
    const race=raceSize();
    const classOverride=globalThis.MICROCOSMOS_CLASS_SIZE_RULES?.[state.cls];
    const subOverride=globalThis.MICROCOSMOS_SUBCLASS_SIZE_RULES?.[state.cls]?.[state.subclass]||globalThis.MICROCOSMOS_SUBCLASS_SIZE_RULES?.[state.subclass];
  ...
  function ensure(){
    ...
    state.sizeProfile.base=base.size;
    state.sizeProfile.baseSource=base.source;
    if(!state.sizeProfile.current||!state.sizeProfile.temporary)state.sizeProfile.current=base.size;
    state.sizeProfile.current=norm(state.sizeProfile.current);
    state.size=state.sizeProfile.current;
    state.creatureSize=state.sizeProfile.current;
  }
  function persist(){try{save()}catch(_e){try{localStorage.setItem("JE_INTEGRATED_123",JSON.stringify(state))}catch(__e){}}}
  function setTemporary(size,source="Efeito temporário"){
    ensure();state.sizeProfile.current=norm(size);state.sizeProfile.temporary=true;state.sizeProfile.currentSource=source;state.size=state.sizeProfile.current;state.creatureSize=state.sizeProfile.current;persist();render();return state.sizeProfile.current
  }
  ...
  function ensureUi(){
    ensureStyle();if(document.getElementById("microCharacterSizeBox"))return;
    const top=document.querySelector("#p1Page .topinfo,.topinfo");if(!top)return;
    const box=document.createElement("div");box.id="microCharacterSizeBox";top.insertAdjacentElement("afterend",box)
  }
  function render(){
    ensure();ensureUi();const box=document.getElementById("microCharacterSizeBox");if(!box)return;
    const p=state.sizeProfile,r=RULES[p.current]||RULES["Médio"];
    box.innerHTML=`<span>📐 <b>Tamanho Base:</b> ${p.base} <small>(${p.baseSource})</small></span><span class="${p.temporary?"micro-size-temp":""}"><b>Tamanho Atual:</b> ${p.current}${p.temporary?` • ${p.currentSource||"temporário"}`:""}</span><span><b>Grid:</b> ${r.square} • ${r.hex}</span>`
  }
  ...
  const obs=new MutationObserver(()=>{ensureUi();render()});obs.observe(document.body,{childList:true,subtree:true});
