/* MICROCOSMOS — Executor automático de combate da Mesa v1.1.
   Valida alcance, rola ataque/Salvaguarda, calcula dano/cura e aplica PV.
   v1.1 adiciona pontos de interrupção para Reações Especiais:
   alvo de ataque → acerto → antes de aplicar dano. */
(async function(){
  ...
  function distance(a,b){const s=+$("gridSize")?.value||70,dx=Math.abs((+a.x||0)-(+b.x||0)),dy=Math.abs((+a.y||0)-(+b.y||0)),type=$("gridType")?.value||"square";if(type==="square")return Math.max(dx,dy)/s*gridMeters();return Math.hypot(dx,dy)/s*gridMeters()}
  function parseRange(item){if(item?.kind==="weapon"&&item.range?.normal)return +item.range.normal;const raw=String(item?.range||"").toLowerCase();if(/pessoal|self/.test(raw))return 0;if(/toque|touch/.test(raw))return 1.5;let m=raw.match(/(\d+(?:[,.]\d+)?)\s*m\b/);if(m)return +m[1].replace(",",".");m=raw.match(/(\d+(?:[,.]\d+)?)\s*(?:ft|pés|pes)/);if(m)return +m[1].replace(",",".")*.3;return item?.kind==="weapon"?1.5:18}
  ...
  async function consumeSlot(caster,item){if((+item.lvl||0)<=0)return{ok:true,level:0};const found=findSlot(caster,+item.lvl||1);if(!found)return{ok:false,reason:"Sem Slot Mágico disponível"};found.row.used=(+found.row.used||0)+1;try{const local=JSON.parse(localStorage.getItem(SHEET_KEY)||"null");if(local&&(!caster.userId||caster.userId===session?.user?.id)){local.magic=local.magic||{};local.magic.slots=local.magic.slots||[];const row=local.magic.slots[found.level]||local.magic.slots[found.level-1];if(row)row.used=(+row.used||0)+1;localStorage.setItem(SHEET_KEY,JSON.stringify(local))}}catch(e){}return{ok:true,level:found.level}}
  async function reactionEvent(event){if(globalThis.MICROCOSMOS_REACTION_CONTEXT)return event;const emit=globalThis.MICROCOSMOS_REACTION_API?.emit;if(typeof emit!=="function")return event;try{return await emit(event)||event}catch(e){console.warn("MICROCOSMOS gatilho de Reação:",e);return event}}

  async function start(caster,type,index){if(!canControl(caster)){log(`🔒 Você não controla <b>${esc(caster.name)}</b>.`);return}if(caster.linked)await globalThis.MICROCOSMOS_TABLE_COMBAT_DATA?.refresh?.();const item=(type==="attack"?caster.attacks:caster.spells)?.[index];if(!item)return;if(!caster.combatDataReady&&caster.linked){log(`⏳ <b>${esc(caster.name)}</b>: aguardando dados completos da ficha. Atualize os tokens e tente novamente.`,caster.color);globalThis.MICROCOSMOS_TABLE_COMBAT_DATA?.refresh?.();return}pending={caster,item,type,index};document.body.classList.add("micro-auto-target");const st=$("mapStatus");if(st)st.textContent=`🎯 ${item.name}: selecione o alvo. Alcance: ${parseRange(item)} m.`}
  ...
  async function hpEffect(caster,target,amount,effect,item,details){
    ...
    if(supabase&&session&&(characterTarget||tokenTarget)){
      ...
      const rpcTarget=characterTarget?target.userId:session.user.id;const {data,error}=await supabase.rpc("request_interaction",{target:rpcTarget,interaction_kind:"combat_effect",interaction_payload:payload});if(error){log(`${details}<br>⚠️ Não foi possível enviar a alteração de PV: ${esc(error.message)}`,caster.color);return}
      if(profile?.role==="master"){const {error:reviewError}=await supabase.rpc("review_interaction",{interaction_id:data,approve:true,note:"Aplicado automaticamente por ação do Mestre"});if(reviewError){log(`${details}<br>⚠️ A ação foi criada, mas não pôde ser aplicada automaticamente: ${esc(reviewError.message)}`,caster.color);return}log(`${details}<br>${effect==="healing"?"💚 Cura":"💥 Dano"} de <b>${amount} PV</b> aplicado diretamente pelo Mestre.`,caster.color);if(tokenTarget)await globalThis.MICROCOSMOS_MESA_SHARED?.reloadTokens?.();else {await globalThis.MICROCOSMOS_TABLE_COMBAT_DATA?.refresh?.();await globalThis.MICROCOSMOS_MESA_SHARED?.flushToken?.(target.id,true)}return data}
  ...
  async function resolve(target){
    const p=pending;if(!p)return;cancel();const {caster,type,index}=p,item=(type==="attack"?caster.attacks:caster.spells)?.[index]||p.item;let dist=distance(caster,target);const max=parseRange(item),selfOnly=max===0;
    ...
    if(!selfOnly&&dist>max+.05){const spend=type==="spell"?await consumeSlot(caster,item):{ok:true};if(type!=="spell"||spend.ok)globalThis.MICROCOSMOS_TACTICAL_TURN?.commitAction?.(caster,{type,item,reason:"tentativa fora do alcance"});log(`❌ <b>${esc(item.name)}</b>: alvo a ${dist.toFixed(1)} m, fora do alcance de ${max} m.${type==="spell"&&+item.lvl>0?(spend.ok?` O Slot de ${spend.level}º Círculo foi gasto pela tentativa.`:` ${spend.reason}.`):" A ação foi gasta."}`,caster.color);return}
    ...
    let targeted=await reactionEvent({kind:"targeted",caster,target,item,type,detail:`${target.name||"Alvo"} foi escolhido como alvo de ${item.name||"uma ação"}.`,acBonus:0});
    if(targeted?.canceled){log(`🌟 <b>${esc(item.name)}</b> foi interrompido por uma Reação.`,caster.color);return}target=targeted?.target||target;dist=distance(caster,target);vfx(caster,target,item.damageType||item.name);

    let success=true,crit=false,rollInfo="",attackD20=0,attackTotal=0,targetAc=(+target.ac||10)+(+targeted?.acBonus||0);
    if(item.attack||type==="attack"){
      attackD20=1+Math.floor(Math.random()*20);attackTotal=attackD20+(+item.bonus||0);crit=attackD20===20;success=crit||attackD20!==1&&attackTotal>=targetAc;rollInfo=`🎲 Ataque: <b>${attackD20} ${(+item.bonus||0)>=0?"+":""}${+item.bonus||0} = ${attackTotal}</b> contra CA <b>${targetAc}</b> → <b>${success?crit?"CRÍTICO":"ACERTO":"ERRO"}</b>`;
      if(success){const hit=await reactionEvent({kind:"hit",caster,target,item,type,roll:attackD20,total:attackTotal,ac:targetAc,acBonus:0,detail:`${target.name||"Alvo"} foi atingido por ${item.name||"um ataque"}.`});if(hit?.canceled)success=false;else{target=hit?.target||target;const reactedAc=(+target.ac||10)+(+targeted?.acBonus||0)+(+hit?.acBonus||0);if(reactedAc!==targetAc){targetAc=reactedAc;success=crit||attackD20!==1&&attackTotal>=targetAc;rollInfo+=`<br>🌟 CA após Reação: <b>${targetAc}</b> → <b>${success?"ACERTO mantido":"ATAQUE BLOQUEADO"}</b>`}}}
    }else if(item.save){const ability=String(item.save).toUpperCase().slice(0,3),d20=1+Math.floor(Math.random()*20),bonus=saveBonus(target,ability),total=d20+bonus,dc=+item.spellDc||+caster.spellDc||10;success=total<dc;rollInfo=`🛡️ Salvaguarda de ${ability}: <b>${d20} ${bonus>=0?"+":""}${bonus} = ${total}</b> contra CD <b>${dc}</b> → <b>${success?"FALHA":"SUCESSO"}</b>`}else rollInfo="✨ Efeito automático no alvo válido.";

    let amount=0,effect="damage",secondaryEffects=[];
    ...
    if(effect==="damage"&&amount>0){const beforeDamage=await reactionEvent({kind:"before_damage",caster,target,item,type,amount,damageType:item.damageType||"",secondaryEffects:[],detail:`${target.name||"Alvo"} sofreria ${amount} de dano de ${item.name||"uma ação"}.`});target=beforeDamage?.target||target;amount=Math.max(0,+beforeDamage?.amount||0);secondaryEffects=Array.isArray(beforeDamage?.secondaryEffects)?beforeDamage.secondaryEffects:[]}
  ...
  await connect();globalThis.MICROCOSMOS_COMBAT_EXECUTOR={start,cancel,resolve,distance,parseRange};
})();
