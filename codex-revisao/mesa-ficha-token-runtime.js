...
(async function(){
  ...
  async function toggleTokenVisibility(p){if(currentRole!=="master"||!p)return;p.visibilityLayer=(p.visibilityLayer||p.layer)==="master"?"players":"master";delete p.layer;saveTable();api.renderPlayers();api.renderTokens();decorateTokens();api.selectToken(p.id);const card=$("tokenCard");card?.querySelector(".micro-token-tools")?.remove();decorateCard();await globalThis.MICROCOSMOS_MESA_SHARED?.flushToken?.(p.id,true);await globalThis.MICROCOSMOS_MESA_SHARED?.reloadTokens?.();api.selectToken(p.id);$("tokenCard")?.querySelector(".micro-token-tools")?.remove();decorateTokens();decorateCard()}
