...
(async function(){
  ...
  function decorateTokens(){
    document.querySelectorAll("#tokenLayer [data-token]").forEach(el=>{const p=players.find(x=>x.id===el.dataset.token);if(!p)return;el.querySelector(".micro-token-photo")?.remove();el.classList.toggle("micro-token-image",!!p.tokenImage);el.classList.toggle("micro-token-master-hidden",currentRole==="master"&&(p.visibilityLayer||p.layer)==="master");if(p.tokenImage){const img=document.createElement("img");img.className="micro-token-photo";img.src=p.tokenImage;img.alt="";el.prepend(img)}});resizeImageTokens();decoratePlayers()
  }
  ...
  function decorateCard(){
    ...
    tools.querySelector("[data-token-visibility]")?.addEventListener("click",()=>toggleTokenVisibility(p));
  ...
  let visibilityHold=null;document.addEventListener("pointerdown",e=>{const token=e.target.closest?.("#tokenLayer [data-token]");if(currentRole!=="master"||!token||!['touch','pen'].includes(e.pointerType))return;const start={x:e.clientX,y:e.clientY,id:token.dataset.token,pointer:e.pointerId};visibilityHold={...start,timer:setTimeout(()=>{const p=players.find(x=>String(x.id)===String(start.id));visibilityHold=null;if(p)showVisibilityMenu(p,start.x,start.y)},650)}},true);document.addEventListener("pointermove",e=>{if(visibilityHold?.pointer===e.pointerId&&Math.hypot(e.clientX-visibilityHold.x,e.clientY-visibilityHold.y)>8){clearTimeout(visibilityHold.timer);visibilityHold=null}},true);for(const eventName of ["pointerup","pointercancel"])document.addEventListener(eventName,e=>{if(visibilityHold?.pointer===e.pointerId){clearTimeout(visibilityHold.timer);visibilityHold=null}},true);
