...
(async function(){
  ...
  function sheetDarkvision(data){const race=globalThis.CODEX_RACE_DATA?.[data.race],texts=[...(race?.features||[]),...Object.values(race?.featureDetails||{}),race?.subraceDetails?.[data.subrace]?.effect||""];for(const value of texts){const match=String(value||"").match(/Vis[aã]o no Escuro\s*(\d+(?:[.,]\d+)?)\s*m/i);if(match)return parseFloat(match[1].replace(",","."))||0}return 0}
  ...
  function sheetToToken(row,index=0){
    ...
    return {
      ...
      linked:true,userId,characterId,tokenImage,tokenImageMode,sheetPortrait,free:false,raceKey:data.race||"",appearance:data.appearance||{},racialScythes:data.race==="race-5"?(existing?.racialScythes||data.appearance?.racialScythes||"recolhidas"):"recolhidas",visionRangeMeters:existing?.visionRangeMeters||9,darkvisionMeters:existing?.darkvisionMeters??sheetDarkvision(data),lightVisionMode:existing?.lightVisionMode||(sheetDarkvision(data)>0?"darkvision":"normal"),visionArcDeg:existing?.visionArcDeg||180,visionDirectionDeg:Number.isFinite(+existing?.visionDirectionDeg)?+existing.visionDirectionDeg:0
    }
  }
  ...
  function decorateTokens(){
    document.querySelectorAll("#tokenLayer [data-token]").forEach(el=>{const p=players.find(x=>x.id===el.dataset.token);if(!p)return;el.querySelector(".micro-token-photo")?.remove();el.querySelector(".micro-token-racial-overlay")?.remove();el.querySelector(".micro-token-visibility-quick")?.remove();const hidden=(p.visibilityLayer||p.layer)==="master";el.classList.toggle("micro-token-image",!!p.tokenImage);el.classList.toggle("micro-token-master-hidden",currentRole==="master"&&hidden);if(p.tokenImage){const img=document.createElement("img");img.className="micro-token-photo";img.src=p.tokenImage;img.alt="";el.prepend(img);const overlays={ativacao:"assets/characters/mantideo-foices-magicas-overlay-ativacao-v1.png",guarda:"assets/characters/mantideo-foices-magicas-overlay-guarda-v1.png",golpe:"assets/characters/mantideo-foices-magicas-overlay-golpe-v1.png"},src=overlays[p.racialScythes];if(src){const effect=document.createElement("img");effect.className="micro-token-racial-overlay";effect.dataset.state=p.racialScythes;effect.src=src;effect.alt="Foices Mágicas Retráteis";el.append(effect)}}});resizeImageTokens();decoratePlayers()
  }
  ...
  async function toggleTokenVisibility(p){if(currentRole!=="master"||!p||!supabase)return false;const previous=p.visibilityLayer||p.layer||"players",next=previous==="master"?"players":"master";p.visibilityLayer=next;delete p.layer;saveTable();api.renderPlayers();api.renderTokens();decorateTokens();const payload={...p,visibilityLayer:next};delete payload.layer;const {error}=await supabase.from("mesa_tokens").upsert({session_key:MESA_SESSION_KEY,token_id:String(p.id),owner_user_id:p.userId||p.ownerUserId||currentUserId,layer:next,data:payload,updated_by:currentUserId,updated_at:new Date().toISOString()},{onConflict:"session_key,token_id"});if(error){p.visibilityLayer=previous;saveTable();api.renderPlayers();api.renderTokens();decorateTokens();alert("Não foi possível alterar a visibilidade: "+error.message);return false}api.selectToken(p.id);$("tokenCard")?.querySelector(".micro-token-tools")?.remove();decorateCard();return true}
  ...
  function decorateCard(){
    ...
    tools.querySelectorAll("[data-scythe-state]").forEach(button=>button.addEventListener("click",()=>{p.racialScythes=button.dataset.scytheState;saveTable();api.renderTokens();decorateTokens();globalThis.MICROCOSMOS_MESA_SHARED?.flushToken?.(p.id,true);tools.remove();decorateCard()}));
  ...
  document.addEventListener("contextmenu",e=>{const token=e.composedPath?.().find?.(node=>node?.matches?.("#tokenLayer [data-token]"))||e.target.closest?.("#tokenLayer [data-token]");if(currentRole!=="master"||!token)return;const p=players.find(x=>String(x.id)===String(token.dataset.token));if(!p)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();showVisibilityMenu(p,e.clientX,e.clientY)},true);
