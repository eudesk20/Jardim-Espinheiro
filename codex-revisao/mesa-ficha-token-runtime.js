...
(async function(){
  ...
  function decorateTokens(){
    document.querySelectorAll("#tokenLayer [data-token]").forEach(el=>{const p=players.find(x=>x.id===el.dataset.token);if(!p)return;el.querySelector(".micro-token-photo")?.remove();el.querySelector(".micro-token-visibility-quick")?.remove();const hidden=(p.visibilityLayer||p.layer)==="master";el.classList.toggle("micro-token-image",!!p.tokenImage);el.classList.toggle("micro-token-master-hidden",currentRole==="master"&&hidden);if(p.tokenImage){const img=document.createElement("img");img.className="micro-token-photo";img.src=p.tokenImage;img.alt="";el.prepend(img)}if(currentRole==="master"){const quick=document.createElement("button");quick.type="button";quick.className="micro-token-visibility-quick";quick.textContent=hidden?'👁':'🎭';quick.title=hidden?'Revelar aos jogadores':'Ocultar dos jogadores';quick.addEventListener("pointerdown",e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();toggleTokenVisibility(p)},{capture:true});quick.addEventListener("click",e=>{e.preventDefault();e.stopPropagation()});el.appendChild(quick)}});resizeImageTokens();decoratePlayers()
  }
