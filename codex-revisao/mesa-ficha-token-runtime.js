...
(async function(){
  ...
  function saveTable(){
    try{
      localStorage.setItem(TABLE_KEY,JSON.stringify(players.map(p=>({
        ...
        tokenImage:p.tokenImage||"",tokenImageMode:p.tokenImageMode||"",sheetPortrait:p.sheetPortrait||""
      }))))
  ...
  function decorateTokens(){
    document.querySelectorAll("#tokenLayer [data-token]").forEach(el=>{const p=players.find(x=>x.id===el.dataset.token);if(!p)return;el.querySelector(".micro-token-photo")?.remove();el.classList.toggle("micro-token-image",!!p.tokenImage);if(p.tokenImage){const img=document.createElement("img");img.className="micro-token-photo";img.src=p.tokenImage;img.alt="";el.prepend(img)}});resizeImageTokens();decoratePlayers()
  }
