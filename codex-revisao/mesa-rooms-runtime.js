...
(function(){
  ...
  function styles(){
    ...
    const style=document.createElement("style");style.id="microRoomsStyle";style.textContent=`
      ...
      .micro-room-head{display:flex;justify-content:space-between;gap:10px;align-items:start}.micro-room-head h2{margin:0;color:#405d3e}.micro-room-tabs,.micro-room-actions{display:flex;gap:7px;flex-wrap:wrap}.micro-room-panel button,.micro-room-panel input,.micro-room-panel select{font:inherit;border:1px solid #8c7552;border-radius:9px;padding:9px;background:#fff8e7;color:#30271e}.micro-room-panel button{font-weight:bold;cursor:pointer}.micro-room-panel button.primary{background:#356342;color:#fff}.micro-room-panel button.dark{background:#25283d;color:#fff}.micro-room-form{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}.micro-room-form label{display:grid;gap:3px;font-size:.8rem}.micro-room-form .wide{grid-column:1/-1}.micro-room-list{display:grid;gap:8px;margin:12px 0}.micro-room-card{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center;background:#fff8e7;border:1px solid #aa9472;border-radius:12px;padding:10px}.micro-room-code{font-family:monospace;letter-spacing:.08em}.micro-room-error{color:#873c38;min-height:1.2em;margin:8px 0}.micro-room-muted{color:#75654e;font-size:.78rem}@media(max-width:620px){.micro-room-overlay{align-items:end;padding:0}.micro-room-panel{border-radius:18px 18px 0 0;max-height:88vh}.micro-room-form{grid-template-columns:1fr}.micro-room-form .wide{grid-column:auto}.micro-room-launch{bottom:70px}}
  ...
  (async()=>{
    try{
      ...
      const active=(rooms||[]).find(r=>r.room_id===saved?.roomId);
      ...
      else if(active){const canMaster=["owner","master"].includes(active.member_role)&&active.master_mode!=="ai";finish({room:active,membership:active,legacy:false,canMaster,client,session})}
      else openPicker(client,rooms||[],profile,{required:true});
      const launch=document.createElement("button");launch.className="micro-room-launch";launch.textContent=active?`🏰 ${active.room_name}`:"🏰 Salas";launch.onclick=()=>openPicker(client,rooms||[],profile);document.body.appendChild(launch);
